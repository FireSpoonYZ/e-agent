...
fn transcript(app: &AppState) -> Text<'static> {
    ...
    if !app.thinking.is_empty() {
        ...
        lines.extend(markdown_text(&app.thinking).lines);
    }
    ...
    if !app.partial.is_empty() {
        ...
        lines.extend(markdown_text(&app.partial).lines);
        ...
        .filter(|tool| matches!(tool.status.as_str(), "running" | "updating"))
    {
        ...
        lines.extend(
            markdown_text(&format!("```typescript\n{}\n```", tool_source(&tool.input))).lines,
        );
...
fn message_lines(message: &Message) -> Vec<Line<'static>> {
    ...
    for content in message.content() {
        match content {
            MessageContent::Text { text } => lines.extend(markdown_text(text).lines),
            MessageContent::Thinking { thinking, .. } => {
                ...
                lines.extend(markdown_text(thinking).lines);
            }
...
mod tests {
    ...
    fn incomplete_markdown_never_panics() {
        for source in ["## Head", "ing\n\n```ru", "st\nfn main() {}\n```"] {
            assert!(!markdown_text(source).lines.is_empty());
        }
    }
