...
async fn main() -> Result<()> {
    ...
    session
        .run_one_trun(UserMessage::text("帮我搜索、总结一下，历史上的今天"))
        .await?;
    session
        .run_one_trun(UserMessage::text(
            "根据今天北京的天气，给我推荐一下今天北京的旅游景点",
        ))
        .await?;
...
// const SYSTEM_PROMPT: &str = r#"你是运行在 e（一套 coding agent harness，编码代理宿主程序）中的专家级编码助手。你通过读取文件、执行命令、编辑代码以及写入新文件来帮助用户。"#;
// const USER_PROMPT: &str = "帮我搜索一下，历史上的今天";
...
//     let mut tool_executor = tool::ProgrammaticToolExecutor::default();
//     let tool_paths = std::env::var_os("E_AGENT_TOOL_PATHS")
//         .ok_or_else(|| error::Error::Config("E_AGENT_TOOL_PATHS is not set".to_string()))?;
//     let tool_paths: Vec<_> = std::env::split_paths(&tool_paths).collect();
//     for path in tool_paths {
...
// async fn run_tool(
//     executor: &tool::ProgrammaticToolExecutor,
//     code: &std::ffi::CStr,
// ) -> anyhow::Result<String> {
//     executor.call(code).await
// }
