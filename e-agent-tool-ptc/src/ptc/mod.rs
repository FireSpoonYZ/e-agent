...
mod tests {
    ...
    async fn exposes_node_and_runs_typescript_with_native_module() {
        ...
        assert!(node_description.contains("single programmatic tool call"));
        assert!(node_description.contains("at most one `node` call per assistant turn"));
        assert!(node_description.contains("Promise.all"));
