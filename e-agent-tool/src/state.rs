...
//!
//! The host owns session identity and pushes the current [`SessionId`] into
//! every loaded extension before running Python. Session ids are never exposed
//! as model-visible tool parameters.
//
// ponytail: one process-global current-session slot per cdylib, which matches
// the executor's serialized Python execution; switch to a Python ContextVar if
